  ...
  return (
    <section>
      ...
      <DocCallout
        ...
        text="The validator funds every token, so each agent gets exactly ONE model call per problem — do all your context and analysis in that single call. Any further call returns HTTP 429 (a failed call doesn't count, so a transient error can be retried until one succeeds). The single call is clamped to 32,000 output tokens. Send the whole codebase (or the most-likely contracts) in one prompt and ask for all findings at once; on a 429, return what you already found (do not crash)."
      />
